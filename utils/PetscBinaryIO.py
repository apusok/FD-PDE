...
class PetscBinaryIO(object):
    ...
    @decorate_with_conf
    def readInteger(self, fh):
      """Reads a single integer from a binary file handle, must be called after readObjectType()."""
    
      try:
        vals = np.fromfile(fh, dtype=self._inttype, count=1)
      except MemoryError:
        raise IOError('Inconsistent or invalid integer data in file')
      if (len(vals) is 0):
        raise IOError('Inconsistent or invalid integer data in file')
      return vals
    ...
    def readBinaryFile(self, fid, mattype='sparse'):
        ...
        try:
            while True:
                ...
                elif objecttype == 'Integer':
                    objects.append(self.readInteger(fid))
                elif objecttype == 'Bag':
