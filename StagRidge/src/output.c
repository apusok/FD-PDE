...
// DoOutputTemp
// ---------------------------------------
PetscErrorCode DoOutputTemp(SolverCtx *sol)
{
  DM             dmVel,  daVel, dmK,  daK, daT,  daRho;
  Vec            vecVel, vaVel, vecK, vaK, vecT, vecRho;
  PetscErrorCode ierr;
  ...
  // Create a new DM and Vec for conductivity
  ierr = DMStagCreateCompatibleDMStag(sol->dmPV,0,0,1,0,&dmK); CHKERRQ(ierr);
  ierr = DMSetUp(dmK); CHKERRQ(ierr);
  
  // Set Coordinates
  ierr = DMStagSetUniformCoordinatesExplicit(dmVel,sol->grd->xmin,sol->grd->xmax,sol->grd->zmin,sol->grd->zmax,0.0,0.0); CHKERRQ(ierr);
  ierr = DMStagSetUniformCoordinatesExplicit(dmK  ,sol->grd->xmin,sol->grd->xmax,sol->grd->zmin,sol->grd->zmax,0.0,0.0); CHKERRQ(ierr);
  
  // Create global vectors
  ierr = DMCreateGlobalVector(dmVel,&vecVel); CHKERRQ(ierr);
  ierr = DMCreateGlobalVector(dmK  ,&vecK  ); CHKERRQ(ierr);
  ...
  // Loop over elements
  {
    ...
    for (j = sz; j < sz+nz; ++j) {
      for (i = sx; i < sx+nx; ++i) {
        ...
        // Calculate element viscosity
        ierr = CalcConductivity_k(sol, &k); CHKERRQ(ierr);

        // Return values in new dm
        row.i = i; row.j = j; row.loc = ELEMENT; row.c = 0;
        ierr = DMStagVecSetValuesStencil(dmK,vecK,1,&row,&k,INSERT_VALUES); CHKERRQ(ierr);
      }
    }
    ...
    ierr = VecAssemblyBegin(vecK); CHKERRQ(ierr);
    ierr = VecAssemblyEnd  (vecK); CHKERRQ(ierr);
  ...
  // Create individual DMDAs for sub-grids of our DMStag objects
  ierr = DMStagVecSplitToDMDA(sol->dmHT,sol->T,ELEMENT, 0,&daT,&vecT); CHKERRQ(ierr);
  ...
  ierr = DMStagVecSplitToDMDA(dmK, vecK,ELEMENT,0,&daK,&vaK); CHKERRQ(ierr);
  ierr = PetscObjectSetName  ((PetscObject)vaK,"Conductivity");              CHKERRQ(ierr);
  ...
  // Dump element-based fields to a .vtr file
  {
    ...
    ierr = PetscViewerVTKOpen(PetscObjectComm((PetscObject)daVel),fname,FILE_MODE_WRITE,&viewer); CHKERRQ(ierr);
    ierr = VecView(vecRho,   viewer); CHKERRQ(ierr);
    ierr = VecView(vaK,      viewer); CHKERRQ(ierr);
    ierr = VecView(vaVel,    viewer); CHKERRQ(ierr);
  ...
  ierr = VecDestroy(&vaK   ); CHKERRQ(ierr);
