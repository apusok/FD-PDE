...
PetscErrorCode FormBCList_HC(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  ...
  for (k=0; k<n_bc; k++) {
    // T = HalfSpaceCoolingTemp(Tm,Ts,-dim_param(x_bc[2*k+1],scalx),kappa,0.0,usr->par->hs_factor); 
    T = MantleAdiabat(usr->par->Tp,usr->nd->A,x_bc[2*k+1]);
  ...
  for (k=0; k<n_bc; k++) {
    if (x_bc[2*k]<=xmor) { // x<=xmor: dH/dz = 0
      value_bc[k] = 0.0;
      type_bc[k] = BC_NEUMANN;
    } else {
...
PetscErrorCode FormBCList_HC_FullRidge(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  ...
  if ((usr->par->forcing == 0) || (usr->par->forcing == 2)) { 
    for (k=0; k<n_bc; k++) {
      ...
      // else T = MantleAdiabat(usr->par->Tp,usr->nd->A,x_bc[2*k+1]); // adiabat
      Hp = (T - usr->par->T0)/usr->par->DT;
      value_bc[k] = Hp; 
      type_bc[k] = BC_DIRICHLET;
    }
...
// ---------------------------------------
#undef __FUNCT__
#define __FUNCT__ "FormBCList_PV_dike"
PetscErrorCode FormBCList_PV_dike(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  UsrData        *usr = (UsrData*)ctx;
  PetscInt       k,n_bc,*idx_bc;
  PetscInt       i, sx, sz, nx, nz, Nx, Nz, iprev, icenter, dm_slot, iphi;
  PetscScalar    *value_bc,*x_bc,*x_bc_stag, xx[2], dx;
  BCType         *type_bc;
  PetscScalar    **coordx,**coordz, ***_xlocal, ***_xEnthlocal;
  Vec            xlocal,xEnthlocal;
  DM             dmEnth;
  PetscFunctionBeginUser;
  ...
  // Get dm and solution vector for phi (Enth)
  ...
  // DIKE
  for (k=0; k<n_bc; k++) {
    for (i = 0; i < usr->par->ncells_dike; i++){
      if (x_bc[2*k+1]==usr->par->dike_z[i]) {
        value_bc[k] = usr->par->fextract*(-usr->par->dike_z[i]/(dx*2.0));
        type_bc[k] = BC_NEUMANN;
      }
    }
  }
...
// ---------------------------------------
#undef __FUNCT__
#define __FUNCT__ "FormBCList_HC_dike"
PetscErrorCode FormBCList_HC_dike(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  UsrData     *usr = (UsrData*)ctx;
  PetscInt    k,n_bc,*idx_bc;
  PetscScalar *value_bc,*x_bc;
