...
const char *DMStagBCLocationNames[] = { "null", "left", "right", "up", "down", 0 };
...
static PetscErrorCode _DMStagBCFillCoords(DM dm,PetscScalar **cx,PetscScalar **cz,DMStagStencilLocation loc,PetscInt i, PetscInt j,DMStagBC *bc)
{
  PetscInt       ii=0,jj=0,iprev=-1,inext=-1,icenter=-1;
  PetscErrorCode ierr;
  ...
  ierr = DMStagGet1dCoordinateArraysDOFRead(dm,&coordx,&coordz,NULL);CHKERRQ(ierr);
  {
    ...
    for (k=0; k<3; k++) {
      for (ii = 0; ii<size[k]; ii++) {
        DMStagBC *b = &bcs[k][ii];
        ierr = _DMStagBCFillCoords(dm,coordx,coordz,b->point.loc,b->point.i,b->point.j,b);CHKERRQ(ierr);
      }
    }
  }
  ierr = DMStagRestore1dCoordinateArraysDOFRead(dm,&coordx,&coordz,NULL);CHKERRQ(ierr);
...
PetscErrorCode DMStagBCListView(DMStagBCList list)
{
  ...
  for (i=0; i<list->nbc_face; i++) {
    PetscPrintf(PETSC_COMM_SELF,"    [%D] x,y (%+1.2e,%+1.2e) i,j (%D %D) side %s\n",i,bc[i].coord[0],bc[i].coord[1],bc[i].point.i,bc[i].point.j,DMStagBCLocationNames[(int)(bc[i].location)]);
  }
