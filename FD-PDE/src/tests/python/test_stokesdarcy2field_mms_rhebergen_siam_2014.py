...
for al in alpha:
    for nx in n:
      ...
      i = alpha.index(al)
      fout1 = f1+'_'+str(nx)+'_'+str(i)+'.out'
    
      # Run with different resolutions
      str1 = '../test_stokesdarcy2field_rhebergen-siam-2014.app -pc_type lu -pc_factor_mat_solver_type umfpack -alpha '+str(al)+' -nx '+str(nx)+' -nz '+str(nx)+' > '+fout1
      print(str1)
      os.system(str1)
      ...
      cbar = fig.colorbar(im,ax=ax, shrink=0.75)
      plt.savefig(f1+'_nx_'+str(nx)+'_alpha_'+str(al)+'.pdf')
      plt.close()
...
for j in range(0, len(alpha)):
    ...
    plt.plot(np.log10(hx),np.log10(nrm1v),lc[j]+'+--',label='v:'+str(alpha[j]))
    plt.plot(np.log10(hx),np.log10(nrm1p),lc[j]+'o--')
