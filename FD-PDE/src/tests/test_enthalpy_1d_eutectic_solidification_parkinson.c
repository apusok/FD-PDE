...
PetscErrorCode Form_Enthalpy(FDPDE fd,PetscInt i,PetscInt j,PetscScalar H,PetscScalar C[],PetscScalar *_P,PetscScalar *_TP,PetscScalar *_T,PetscScalar *_phi,PetscScalar *CF,PetscScalar *CS,PetscInt ncomp, void *ctx) 
{
  ...
  // if (i==2) PetscPrintf(PETSC_COMM_WORLD,"# EUT: j=%d H=%f [Hsol,Heut,Hliq]=[%f %f %f]\n",j,H,Hsol,Heut,Hliq);
  ...
  if (H <= Hsol) { // below solidus
    ...
    T = -CF[0];
    // T = eval_T(H,phi,S,cp); 
    // if (i==2) PetscPrintf(PETSC_COMM_WORLD,"# MUSH: j=%d H=%f phi=%f CS=%f CF=%f T=%f\n",j,H,phi,CS[0],CF[0],T);
  } else { // above liquidus
    phi = 1.0;
    ...
    // if (i==2) PetscPrintf(PETSC_COMM_WORLD,"# liquid: j=%d H=%f phi=%f CS=%f CF=%f T=%f\n",j,H,phi,CS[0],CF[0],T);
  }
...
PetscErrorCode ApplyBC_Enthalpy(DM dm, Vec x, PetscScalar ***ff, void *ctx)
{
  ...
  thE  = 0.0;
  C_top = usr->par->C0; 
  phiE = eval_phiEutectic(C_top,usr->par->C);
  H_top = phiE*usr->par->S+thE;

  // bottom
  thdown = 1.0+analytical_theta_liquid(usr->par->th_inf,usr->par->th_i,usr->par->zmin,usr->par->h0);
...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  ierr = PetscBagRegisterScalar(bag, &par->C, 5.0, "C", "Eutectic compositional number [-]"); CHKERRQ(ierr);
  ...
  ierr = PetscBagRegisterScalar(bag, &par->Csi, 5.0, "Csi", "Solid composition for analytical solution [-]"); CHKERRQ(ierr);
