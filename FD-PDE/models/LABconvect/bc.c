...
// ---------------------------------------
// FormBCList_PV_SidesPeriodic_InflowOutflowBottom
// ---------------------------------------
#undef __FUNCT__
#define __FUNCT__ "FormBCList_PV_SidesPeriodic_InflowOutflowBottom"
PetscErrorCode FormBCList_PV_SidesPeriodic_InflowOutflowBottom(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  UsrData        *usr = (UsrData*)ctx;
  PetscInt       k,n_bc,*idx_bc;
  PetscInt       i, sx, sz, nx, nz, Nx, Nz, iprev, icenter;
  ...
  // DOWN no slip Vx = 0, (point not on true boundary)
  ierr = DMStagBCListGetValues(bclist,'s','-',PV_FACE_VS,&n_bc,&idx_bc,&x_bc,&x_bc_stag,&value_bc,&type_bc);CHKERRQ(ierr);
  for (k=0; k<n_bc; k++) {
    value_bc[k] = 0.0;
    type_bc[k] = BC_DIRICHLET;
  ...
  // UP no slip Vx = 0 (point not on true boundary)
  ierr = DMStagBCListGetValues(bclist,'n','-',PV_FACE_VS,&n_bc,&idx_bc,&x_bc,&x_bc_stag,&value_bc,&type_bc);CHKERRQ(ierr);
  for (k=0; k<n_bc; k++) {
    value_bc[k] = 0.0;
    type_bc[k] = BC_DIRICHLET; 
...
// ---------------------------------------
// FormBCList_HC_SidesPeriodic_InflowOutflowBottom
// ---------------------------------------
#undef __FUNCT__
#define __FUNCT__ "FormBCList_HC_SidesPeriodic_InflowOutflowBottom"
PetscErrorCode FormBCList_HC_SidesPeriodic_InflowOutflowBottom(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  UsrData     *usr = (UsrData*)ctx;
  DM          dmPV;
  Vec         xPV, xPVlocal;
