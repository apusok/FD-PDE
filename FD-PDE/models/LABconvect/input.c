...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  // domain parameters
  ierr = PetscBagRegisterInt(bag, &par->nx, 40, "nx", "Element count in the x-dir [-]"); CHKERRQ(ierr);
  ...
  ierr = PetscBagRegisterScalar(bag, &par->rho0, 3300, "rho0", "Reference density of matrix and magma [kg/m^3]"); CHKERRQ(ierr);
...
PetscErrorCode InputPrintData(UsrData *usr)
{
  ...
  if (usr->par->start_run) {
    ...
    PetscPrintf(usr->comm,"# Lithosphere-Asthenosphere Boundary BUOYANCY MODEL: %s \n",&(date[0]));
    PetscPrintf(usr->comm,"# --------------------------------------- #\n");
...
PetscErrorCode NondimensionalizeParameters(UsrData *usr)
{
  ...
  nd->age = nd_param(par->age*1.0e6*SEC_YEAR,scal->t);
