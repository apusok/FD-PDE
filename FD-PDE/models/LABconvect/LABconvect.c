// ---------------------------------------
// Lithosphere-Asthenosphere Boundary model solving for conservation of mass, momentum, energy and composition using the Enthalpy Method
// run: ./LABconvect.app -options_file model_test.opts
// ---------------------------------------
static char help[] = "Lithosphere-Asthenosphere Boundary with melt-matrix buoyancy model using a 3-Field formulation \n\n";
...
"  << Stokes-Darcy BCs >> \n"
"  LEFT: free-slip/periodic \n"
"  RIGHT: free-slip/periodic \n"
"  DOWN: free-slip/inflow-outflow \n"
"  UP: free-slip \n";
...
"  << Enthalpy (H) BCs >> \n"
"  LEFT: insulating/periodic \n"
"  RIGHT: insulating/periodic  \n"
"  DOWN: H = Hp, C = C0 \n"
"  UP: H = Hc, C = Cdepl \n";
...
PetscErrorCode Numerical_solution(void *ctx)
{
  ...
  while ((nd->t <= nd->tmax) && (nd->istep <= par->tstep)) {
    ...
    // Solve PV - pressure and velocity
    PetscPrintf(PETSC_COMM_WORLD,"# (PV) Mechanics Solver - Stokes-Darcy3Field \n");
    SNESConvergedReason reason;
    converged = PETSC_FALSE;
    while (!converged) { // this is legacy fix from mbuoy3
