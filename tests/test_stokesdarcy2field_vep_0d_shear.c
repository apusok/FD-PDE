...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  ierr = PetscBagRegisterScalar(bag, &par->phi_0, 1e-4, "phi_0", "Reference porosity"); CHKERRQ(ierr);
...
PetscErrorCode FormBCList(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  ...
  */
  // pin pressure dof
  ierr = DMStagBCListPinCornerValue(bclist,DMSTAG_DOWN_LEFT,'o',0,usr->par->P0); CHKERRQ(ierr);
  //ierr = DMStagBCListPinCornerValue(bclist,DMSTAG_UP_LEFT,'o',0,usr->par->P0); CHKERRQ(ierr);
