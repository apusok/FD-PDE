 ...
 ierr = FDPDESetFunctionBCList(fd,FormBCList,bc_description,usr); CHKERRQ(ierr);
  ...
  while ((ickpt <= maxckpt) && (istep<tstep))  {
    ...
    */
    ...
    ierr = VecMax(usr->VMag,NULL, &xxmax); CHKERRQ(ierr);
...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  // Elasticity
  ierr = PetscBagRegisterScalar(bag, &par->Gu, 1e40, "Gu", "Shear elastic modulus"); CHKERRQ(ierr);
  ierr = PetscBagRegisterScalar(bag, &par->Gd, 1e40, "Gd", "Shear elastic modulus"); CHKERRQ(ierr);
