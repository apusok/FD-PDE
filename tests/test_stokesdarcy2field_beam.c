// ---------------------------------------
// Benchmark: a visco-elastic bending beam
...
// run: ./test_stokesdarcy2field_beam_ -nx 100 -nz 100 -pc_type lu -pc_factor_mat_solver_type umfpack -log_view
...
typedef struct {
  ...
  PetscScalar    lam_v, eta_d, eta_u, zeta_d, zeta_u, C_u, C_d, Fu, Fd, Gu, Gd, z_in;
  PetscScalar    lambda,lam_p,Z,q,R,phi_0,n,nh;
  ...
  */
  ...
  if (fchk < aa ) {
    ...
    if (PetscAbs(tx)-1.0 > tol) {
      PetscCall(PetscPrintf(PETSC_COMM_WORLD, "tx = %1.4f\n", tx));}
...
PetscErrorCode StokesDarcy_Numerical(void *ctx)
{
 ...
 PetscCall(FDPDESetFunctionBCList(fd,FormBCList,bc_description,usr)); 
  ...
  //while (istep<tstep) {
  //while (ickpt < maxckpt) {
  while ((ickpt <= maxckpt) && (istep<tstep))  {
...
#if 1
    {
      ...
      PetscCall(PetscPrintf(PETSC_COMM_WORLD, "hk1norm=%g, hk2norm=%g, hk3norm=%g, hk4norm=%g \n", hk1norm, hk2norm, hk3norm,hk4norm));
    ...
    */
    ...
    PetscCall(VecMax(usr->VMag,NULL, &xxmax)); 
    usr->par->gamma = xxmax; 
    if (usr->par->gamma < 1e-5) {usr->par->gamma = 1e-5;}
    //change dt accordingly
    dtt = 1.0/nx/usr->par->gamma/2.0; //maximum time step allowed for boundedness
    dtgap = tckpt - usr->par->t;   // gap between the current time and the next checkpoint

    if (usr->par->t >= 0.04) {usr->par->Fu = 0.0; usr->par->Fd = 0.0;} //turn off gravity
    ...
    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "Phase field: Maximum of U = %1.8f \n", xxmax)); 
...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  // Elasticity
  PetscCall(PetscBagRegisterScalar(bag, &par->Gu, 1e40, "Gu", "Shear elastic modulus")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->Gd, 1e40, "Gd", "Shear elastic modulus")); 
...
PetscErrorCode FormCoefficient(FDPDE fd, DM dm, Vec x, DM dmcoeff, Vec coeff, void *ctx)
{
  ...
  //  PetscCall(PetscPrintf(PETSC_COMM_WORLD, "fu, fd = %g, %g", F_u, F_d));
  ...
  for (j = sz; j < sz+nz; j++) {
    for (i = sx; i <sx+nx; i++) {
      ...
      { // A = eta (center, c=1), and also compute chi_s and chi_p (center, c = 4,5)
        ...
        //        if (ff>=0.5) {eta_v = eta_u; zeta_v = zeta_u; Y = C_u; G = G_u;}
        //else {eta_v = eta_d; zeta_v = zeta_d; Y = C_d; G = G_d;}
      ...
      { // A = eta (corner, c=0)
        ...
        for (ii = 0; ii < 4; ii++) {
          ...
          //          if (ff[ii]>=0.5) {eta_v = eta_u; zeta_v = zeta_u; Y[ii] = C_u; G = G_u;}
          //else {eta_v = eta_d; zeta_v = zeta_d; Y[ii] = C_d; G = G_d;}
          ...
          //PetscCall(PetscPrintf(PETSC_COMM_WORLD, "A (corner) = %g \n", c[j][i][idx])); 
      ...
      { // B = phi*F*ek - grad(chi_s*tau_old) + div(chi_p*dP_old) (edges, c=0)
        ...
        //        PetscCall(PetscPrintf(PETSC_COMM_WORLD, "i,j = %d, %d; ff = %g, %g, %g, %g \n", i,j,ff[0], ff[1], ff[2], ff[3])); 
        ...
        for (ii = 0; ii < 4; ii++) {
          ...
          //                    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "B = %g \n", c[j][i][idx])); 
        }
      }
      ...
      { // D3 = R^2 * Kphi * F (edges, c=2)
        ...
        for (ii = 2; ii < 4; ii++) {
          ...
          //          PetscCall(PetscPrintf(PETSC_COMM_WORLD, "D3 = %g \n", c[j][i][idx])); 
        }
      }
    }
  }
...
PetscErrorCode FormBCList(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  ...
  for (k=0; k<n_bc; k++) {
    value_bc[k] = 0.0;
    type_bc[k] = BC_DIRICHLET_STAG; //BC_NEUMANN;//BC_DIRICHLET;
  }
...
PetscErrorCode SetInitialField(DM dm, Vec x, void *ctx)
{
  ...
  for (j = sz; j < sz+nz; j++) {
    for (i = sx; i <sx+nx; i++) {
      DMStagStencil point;
      PetscScalar   xp,zp, fval = 0.0, xn, zw,  bw, bl, xo, zo;
      PetscInt      idx;
      ...
      if (xp <= bl) {
        if (zp >= zw) { xn = zp - (zw + 0.5*bw);}
        if (zp < zw) { xn = (zw-0.5*bw) - zp;}
      }
      ...
      if (xp > bl) {
        xn = PetscPowScalar(PetscPowScalar(xp-xo, 2) + PetscPowScalar(zp-zo, 2), 0.5) - 0.5*bw;
      }
