...
"  << Stokes-Darcy BCs >> \n"
"  CASE 2 -- \n"
"  UP/Bottom: no penetration, free slip, \n"
"  LEFT/RIGHT: outflux with the magnitude of vi, free slip dVz/dx=0 \n"
"  Zero pressure at left, Zero normal graident at other boundaries. \n";
...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  ierr = PetscBagRegisterScalar(bag, &par->vi, 1.0, "vi", "Extension velocity"); CHKERRQ(ierr);
  // Viscosity
  ierr = PetscBagRegisterScalar(bag, &par->eta_v0, 1.0, "eta_v0", "Reference shear viscosity"); CHKERRQ(ierr);
  // Plasticity
  ierr = PetscBagRegisterScalar(bag, &par->C, 1e40, "C", "Yield Creteria"); CHKERRQ(ierr);
  ierr = PetscBagRegisterScalar(bag, &par->lam_p, 1.0, "lam_p", "Multiplier to control the compaction plasticity, zeta_p = eta_p/(lam_p*phi)"); CHKERRQ(ierr);
...
PetscErrorCode FormCoefficient(FDPDE fd, DM dm, Vec x, DM dmcoeff, Vec coeff, void *ctx)
{
  ...
  PetscScalar    dt,F,G,Z0,Z,eta_v0,Y,lam_p,phi,Kphi,eta_v,zeta_v,eta_e,zeta_e;
  PetscErrorCode ierr;
  ...
  phi = 1.0 - (1.0-usr->par->phi_0)*PetscExpScalar(-usr->par->vi * usr->par->t);
  ...
  for (j = sz; j < sz+nz; j++) {
    for (i = sx; i <sx+nx; i++) {
      ...
      { // A = eta (center, c=1), and also compute chi_s and chi_p (center, c = 4,5)
        ...
        // volumetric stress
        //DeltaP = -(zeta - 2.0/3.0*eta)*div+chip*pold; //REMEMBER TO CORRECT TAU_OLD AS WELL
        DeltaP = -zeta*div+chip*pold;
        DeltaP = DeltaP/(1-phi);
      ...
      { // A = eta (corner, c=0)
        ...
        // save stresses
        ierr = DMStagGetLocationSlot(usr->dmeps,DOWN_LEFT,0,&idx); CHKERRQ(ierr); xxs[j][i][idx] = txx[0]; xxy[j][i][idx] = Y;
        ...
        ierr = DMStagGetLocationSlot(usr->dmeps,DOWN_RIGHT,0,&idx); CHKERRQ(ierr); xxs[j][i][idx] = txx[1]; xxy[j][i][idx] = Y;
        ...
        ierr = DMStagGetLocationSlot(usr->dmeps,UP_LEFT,0,&idx); CHKERRQ(ierr); xxs[j][i][idx] = txx[2]; xxy[j][i][idx] = Y;
        ...
        ierr = DMStagGetLocationSlot(usr->dmeps,UP_RIGHT,0,&idx); CHKERRQ(ierr); xxs[j][i][idx] = txx[3]; xxy[j][i][idx] = Y;
