...
// run: ./test_stokesdarcy2field_vp_inclusion_ -pc_type lu -pc_factor_mat_solver_type umfpack -nx 10 -nz 10 -log_view
...
PetscErrorCode StokesDarcy_Numerical(void *ctx)
{
  ...
  //PetscCall(ComputeExtraParameters(dm,usr));  //NOT SURE IF I NEED THIS
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_solution_initial_dim",usr->par->fdir_out,usr->par->fname_out));
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_coefficient_initial",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(dmcoeff,xcoeff,fout));

  PetscCall(ScaleCoefficient(dmcoeff,xcoeff,&xscaled,usr));
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_coefficient_initial_dim",usr->par->fdir_out,usr->par->fname_out));
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_solution",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(dm,x,fout));

  PetscCall(ScaleSolution(dm,x,&xscaled,usr));
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_solution_dim",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(dm,xscaled,fout));
  PetscCall(VecDestroy(&xscaled));

  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_strain",usr->par->fdir_out,usr->par->fname_out));
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_strain_dim",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(usr->dmeps,xscaled,fout));
  PetscCall(VecDestroy(&xscaled));

  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_stress",usr->par->fdir_out,usr->par->fname_out));
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_stress_dim",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(usr->dmeps,xscaled,fout));
  PetscCall(VecDestroy(&xscaled));

  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_yield",usr->par->fdir_out,usr->par->fname_out));
  ...
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_yield_dim",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(usr->dmeps,xscaled,fout));
  PetscCall(VecDestroy(&xscaled));
  ...
  PetscCall(FDPDEGetCoefficient(fd,&dmcoeff,&xcoeff));
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_coefficient",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(dmcoeff,xcoeff,fout));

  PetscCall(ScaleCoefficient(dmcoeff,xcoeff,&xscaled,usr));
  PetscCall(PetscSNPrintf(fout,sizeof(fout),"%s/%s_coefficient_dim",usr->par->fdir_out,usr->par->fname_out));
  PetscCall(DMStagViewBinaryPython(dmcoeff,xscaled,fout));
  PetscCall(VecDestroy(&xscaled));
...
PetscErrorCode InputParameters(UsrData **_usr)
{
  ...
  PetscCall(PetscBagRegisterScalar(bag, &par->phi_s, 0.0, "phi_s", "Porosity amplitude")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->m, 1.0, "m", "Trigonometric coefficient")); 
  ...
  PetscCall(PetscBagRegisterScalar(bag, &par->eta_b, 0.5e3, "eta_b", "Block shear viscosity [Pa.s]")); 
  //PetscCall(PetscBagRegisterScalar(bag, &par->zeta_b, 0.5*5.e3/3.0, "zeta_b", "Compaction viscosity in block: 5/3*eta0")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->eta_w, 1.0e-1, "eta_w", "Weak zone shear viscosity [Pa.s]")); 
  //PetscCall(PetscBagRegisterScalar(bag, &par->zeta_w, 5.0e-1/3.0, "zeta_w", "Compaction viscosity in weak medium: 5/3*eta0")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->lambda, 0.0, "lambda", "Exponential melt weakening factor")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->vi, 1.0, "vi", "Extension/compression velocity [m/s]")); 
  PetscCall(PetscBagRegisterScalar(bag, &par->C_b, 20.0, "C_b", "Block Cohesion [Pa]")); 
  // Identifier to use effective shear viscosity in calculating bulk viscosity or not
  PetscCall(PetscBagRegisterInt(bag, &par->bulk_eff, 0, "bulk_eff", "Bulk viscosity is propotional to effective shear viscosity: 1 yes, 0 no")); 
...
PetscErrorCode InputPrintData(UsrData *usr)
{
  ...
  PetscCall(PetscPrintf(usr->comm,"# Test_stokesdarcy2field_vp_inclusion: %s \n",&(date[0])));
  PetscCall(PetscPrintf(usr->comm,"# --------------------------------------- #\n"));
...
PetscErrorCode FormBCList(DM dm, Vec x, DMStagBCList bclist, void *ctx)
{
  ...
  */
  ...
  //PetscCall(DMStagBCListPinCornerValue(bclist,DMSTAG_UP_RIGHT,'o',0,usr->par->nd_P)); 
