...
def plot_allinone(fname,ft,nx):
  ...
  im = ax.imshow(vx.reshape(mz,mx+1),extent=[min(xv), max(xv), min(zc), max(zc)],vmin=umin,vmax=umax,cmap=cmaps,origin='lower')
  ax.set_title(r'$V_x$')
  cbar = fig.colorbar(im,ax=ax, shrink=0.75)
...
# filenames 
fname = 'out_stokesdarcy2field_vep_0d_expansion'
...
# Run simulation
str1 = 'mpiexec -n '+str(ncpu)+' ../test_stokesdarcy2field_vep_0d_expansion.sh'+ newton + solver + sdpar + \
  ' -output_file '+fname+ \
  ' -C '+str(C)+ \
  ' -etamin '+str(etamin)+ \
...
stress_fdm = np.zeros([tstep+1])
stress_fdm_eff = np.zeros([tstep+1])
...
for istep in range(0,tstep,tout):
  ...
  phi = 1.0 - (1.0-phi0)*np.exp(-vi*tt[istep+1])
  stress[istep+1] = (1-phi)*stress[istep+1]
  stress_ana[istep+1] = -(1-phi)*(vi)*(1.0 - np.exp(-tt[istep+1]*Z0))
