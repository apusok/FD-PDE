...
def test1_diffusion_space(fname,fname_out,n,ncpu):
  ...
  for i in range(len(n)):
    ...
    str1 = 'mpiexec -n '+str(ncpu)+' ../test_advdiff_mms_convergence_ '+solver+' -test 1 -output_file '+fname+ \
...
def test2_advection_diffusion_space(fname,fname_out,n,ncpu):
  ...
  for adv_scheme in nadv_scheme:
    ...
    for i in range(len(n)):
      ...
      str1 = 'mpiexec -n '+str(ncpu)+' ../test_advdiff_mms_convergence_ '+solver+' -test 2 -output_file '+fname+\
...
def test3_timediff(fname,fname_out,dt,tend,n,ncpu):
  ...
  for ts_scheme in nts_scheme:
    ...
    for i in range(len(dt)):
      ...
      # Run test
      str1 = 'mpiexec -n '+str(ncpu)+' ../test_advdiff_mms_convergence_ '+solver+' -test 3'+ \
            ' -dtmax '+str(dtmax)+ \
            ' -tmax '+str(tend)+ \
            ' -tstep '+str(tstep_max)+ \
...
def test4_timeadv(fname,fname_out,dt,tend,n,ncpu):
  ...
  for ts_scheme in nts_scheme:
    ...
    for i in range(len(dt)):
      ...
      # Run test
      str1 = 'mpiexec -n '+str(ncpu)+' ../test_advdiff_mms_convergence_ '+solver+' -test 4'+ \
            ' -dtmax '+str(dtmax)+ \
            ' -tmax '+str(tend)+ \
            ' -tstep '+str(tstep_max)+ \
            ' -ts_scheme '+str(ts_scheme)+ \
...
def test5_advection_diffusion_BC(fname,fname_out,n,bc,ncpu):
  ...
  for adv_scheme in nadv_scheme:
    ...
    for i in range(len(n)):
      ...
      str1 = 'mpiexec -n '+str(ncpu)+' ../test_advdiff_mms_convergence_ '+solver+' -test 5 -output_file '+fname+ \
        ' -output_dir '+fname_data+ \
        ' -adv_scheme '+str(adv_scheme)+ \
        ' -bcleft '+str(int(bc[0]))+ \
