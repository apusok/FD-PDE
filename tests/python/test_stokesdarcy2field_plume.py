...
# Input file
fname = 'out_stokesdarcy2field_plume'
...
model = ' -L '+str(L)+' -H '+str(H) + \
    ' -dt '+str(dt)+' -z_in '+str(z_in) +\
    ' -eta1 '+str(eta1) + ' -eta2 '+str(eta2) + ' -eta3 '+str(eta3) + \
    ' -eta4 '+str(eta4) + \
    ' -C1 '+str(C1) + ' -C2 '+str(C2) + ' -C3 '+str(C3) +\
    ' -C4 '+str(C4) +\
    ' -F1 ' + str(F1) + ' -F2 ' + str(F2) + ' -F3 ' + str(F3) +\
    ' -F4 ' + str(F4) +\
...
# Run test
# Forward euler
str1 = 'mpiexec -n '+str(ncpu)+' ../test_stokesdarcy2field_plume.app' + \
...
# data to store the peak of the surface
hpeak = np.zeros([maxckpt+1,2])
umax = np.zeros([maxckpt+1,2])
iumax = np.zeros([maxckpt+1,2])

for ickpt in range(1,maxckpt+1):
#for ickpt in range(maxckpt,maxckpt+1):
  ...
  hpeak[ickpt,0] = dtck * ickpt
  umax[ickpt,0] = dtck * ickpt
  iumax[ickpt,0] = dtck * ickpt

  # compute the velocity magnitude at the cell center
  umax[ickpt,1] = 0
  for i in range(0,mx):
    for j in range(0,mz):
      vxc[j][i] = 0.5 * (vxface[j][i+1] + vxface[j][i])
      ...
      if(vc[j][i] >= umax[ickpt,1]):
        umax[ickpt,1] = vc[j][i]
        iumax[ickpt,1] = j
      
  
  # find the highest point where the phase value = 0.5 at i = 0
  fc = fres0.reshape(n, m)
  i = 0
  for j in range(mz-1,0,-1):
    if (fc[j][i] < 0.5):
      hpeak[ickpt,1] = zc[j] + (zc[j+1] - zc[j])/(fc[j+1][i] - fc[j][i]) * (0.5 - fc[j][i])
      break;
  
  print('peak at ', hpeak[ickpt,1],j,zc[j],zc[j+1], fc[j][i], fc[j+1][i])

  print('Maximum velocity magnitude = ', np.max(vc), umax[ickpt,1],iumax[ickpt,1])
...
#  if (ickpt == maxckpt-1):
  if ( ickpt % tout2 == 0):
    ...
    # Plot solution 
    fig1, axs1 = plt.subplots(2, 1,figsize=(4,4))
    ...
    # color map
    ax0 = axs1[0]
    im = ax0.imshow(fres0,extent=[min(xc), max(xc), min(zc), max(zc)],cmap=cmaps,origin='lower',aspect='auto')
    #im = ax0.imshow(pp,extent=[min(xc), max(xc), min(zc), max(zc)],cmap=cmaps,origin='lower')
    Q  = ax0.quiver( xc[::nind], zc[::nind], vxc[::nind,::nind], vzc[::nind,::nind], units='width', pivot='mid', scale_units='xy', scale=20 )
    ...
    ax01 = axs1[1]
    ax01.contour( xc , zc , fres0, levels=[0.5] , colors='black',linestyles='solid',linewidths=1.0)
    ax01.set_ylim((0.699, 0.702))
    ax01.set_xlabel('x')
    ax01.set_ylabel('z')
    ax01.grid(True,color='gray', linestyle=':', linewidth=0.5)
...
ax3 = axs3
ax3.plot(hpeak[:,0], hpeak[:,1]-0.7, 'k-')
ax3.set_xlabel('t')
ax3.set_ylabel('Max topography')
ax3.set_xlim((0, 0.3))
ax3.set_ylim((0, 0.0025))
plt.tight_layout()
fig3.savefig(fname+'/'+fname+'_peak.pdf')
plt.close(fig3)

fig4, axs4 = plt.subplots(1,1,figsize=(6,5))
ax4 = axs4
ax4.plot(umax[:,0], umax[:,1], 'k-')
ax4.set_xlabel('t')
ax4.set_ylabel(r'Max($|\mathbf{u}|$)')
ax4.set_xlim((0, 0.3))
ax4.set_ylim((0, 3.5))
plt.tight_layout()
fig4.savefig(fname+'/'+fname+'_umax.pdf')
plt.close(fig4)

fig5, axs5 = plt.subplots(1,1,figsize=(6,5))
ax5 = axs5
ax5.plot(iumax[:,0], 0.8*iumax[:,1]/n, 'k-')
ax5.set_xlabel('t')
ax5.set_ylabel('Vertical position of max(U_mag)')
ax5.set_xlim((0, 0.3))
ax5.set_ylim((0, 0.8))
plt.tight_layout()
fig5.savefig(fname+'/'+fname+'_iumax.pdf')
plt.close(fig5)
