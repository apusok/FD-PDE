...
# Run simulation
str1 = 'mpiexec -n '+str(ncpu)+' ../test_stokesdarcy2field_vep_0d_shear_' + newton + solver + sdpar + \
  ' -output_file '+fname+ \
  ' -C '+str(C)+ \
  ' -etamin '+str(etamin)+ \
...
C = 0.2#1.0
etamin = 0.0
str2 = 'mpiexec -n '+str(ncpu)+' ../test_stokesdarcy2field_vep_0d_shear_' + newton + solver + sdpar + \
  ' -output_file '+fname+ \
  ' -C '+str(C)+ \
  ' -etamin '+str(etamin)+ \
  ' -output_dir '+fname+' -nx '+str(nx)+' -nz '+str(nx)+' > '+fout
