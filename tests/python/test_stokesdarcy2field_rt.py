...
# Input file
fname = 'out_stokesdarcy2field_rt_k1'
...
model = ' -L 0.9142 -H 1.0  -dt '+str(dt)+' -z_in '+str(z_in) +\
...
# Run test
# Forward euler
str1 = 'mpiexec -n '+str(ncpu)+' ../test_stokesdarcy2field_rt_' + \
...
#  if (ickpt == maxckpt-1):
  if ( ickpt % tout2 == 0):
    ...
    ax0.set_title('van Keken benchmark')
    ax0.set_xlabel('x')
    ax0.set_ylabel('z')
    #cbar = fig1.colorbar(im,ax=ax0, shrink=0.75)
    ax0.grid(True,color='gray', linestyle=':', linewidth=0.5)        

    fig1.savefig(fname+'/'+fname+ft+'_rk2.pdf')
    plt.close(fig1)

fig2, axs2 = plt.subplots(1, 1,figsize=(4,4))
ax0 = axs2
ax0.plot(vrms[:,0], vrms[:,1], 'k-')
ax0.set_xlabel('$t$')
ax0.set_ylabel(r'$v_{rms}$')
fig2.savefig(fname+'/'+fname+'_vrms.pdf')

np.savetxt(fname+'/'+'data_vrms.csv', vrms, delimiter=',')
